...
import sys,os
import scipy.io
from softmax import Softmax, ResNetwork, train_with_sgd
...
def run_tests(sample, label, num_labels):
    ...
    softmax_val_x = lambda x: demo_softmax.calc_forward_pass(x, label)
    softmax_grad_x = lambda x: demo_softmax.grad_by_x(x, label)  # returns (dL\dX)
    gradient_test(softmax_val_x, softmax_grad_x, sample, 100, 50, 0.1)

    ## Test softmax gradient w.r.t X numerically ##

    grad = demo_softmax.grad_by_x(sample, label)
    ...
    softmax_val_w = lambda w: FunctionsBoxes.softmax_loss_and_gradient_regularized(w, demo_softmax.add_bias_dimension(sample), label, 0.1)[0]
    softmax_grad_w = lambda w: FunctionsBoxes.softmax_loss_and_gradient_regularized(w, demo_softmax.add_bias_dimension(sample), label, 0.1)[1]
    gradient_test(softmax_val_w, softmax_grad_w, w, 10, 20, 0.1)

    ## Test softmax gradient w.r.t Params numerically ##

    __, grad = demo_softmax.calc_loss_and_grad_for_batch(sample, label)
    ...
    layer_val_x = lambda x: demo_layer.calc_forward_pass(x)
    layer_jacobian_vec_x = lambda x, v: demo_layer.calc_backward_pass(x, v)[-1]  # returns (dL\dX)^T . v
    jacobian_test(layer_val_x, layer_jacobian_vec_x, sample, 100, 20, 0.1)
